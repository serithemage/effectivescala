...
[Scala][Scala]는 트위터에서 사용하고 있는 주요 어플리케이션 프로그래밍 언어중에 하나이다.트위터의 환경 대부분은 스칼라로 작성되어 있을뿐 아니라, 우리들의 업무를 지원하는 [여러 대규모 라이브러리들](http://github.com/twitter/)을 보유하고 있다. 스칼라는 대단히 효과적인 언어지만, 또한 거대한 언어이기도 하다. 우리들의 경험에 의하면, 스칼라를 이용한 개발은 충분한 주의를 필요로 한다. 피해야 할 함정은 없는가? 어떤 기능을 사용하고, 또 사용하지 말아야 할 것인가? 언제 순수 함수형을 사용해야 하고, 또 언제 그걸 피해야 하는가?  즉, 우리가 발견한 스칼라의 효과적인 사용법이란 무엇인가? 본 가이드의 목적은, 우리들의 경험으로부터 발견해낸 일련의 모범사례를 제공하는것이다. 트위터에서는 스칼라를 주로 분산시스템을 구성하는 대용량의 서비스군 작성에 사용하고 있기 때문에, 우리들의 조언 역시 그러한 방향에 초점이 맞춰져 있음을 미리 밝혀둔다. 하지만, 여기에 있는 조언의 대부분은 다른 문제영역에도 자연스럽게 적용 가능할 것이다. 이것은 법칙이 아니다. 따라서 어느정도의 일탈은 넘어가 줄 수 있어야 할 것이다.
...
우선은 스칼라로 프로그래밍을 하자. 당신이 쓰는 언어는 자바도 아니고, 해스켈도 아니며, 파이선도 아니다. 스칼라 프로그래밍은 전술한 다른 언어들과도 다르다. 프로그래밍언어를 효과적으로 사용하기 위해서는, 문제를 그 언어를 이용해 효과적으로 표현할 수 있어야만 한다. 자바 프로그램을 억지로 스칼라로 작성하게 되면 대부분의 경우 원래의 것 보다 떨어지는 결과를 가져올 것이다.

본 가이드는 스칼라 입문서가 아니며, 스칼라에 익숙한 사람을 위해 쓰여졌다. 지금부터 스칼라를 배워보려는 사람에게는 다음과 같은 교재가 있다:

* [Scala School](http://twitter.github.com/scala_school/)
* [Learning Scala](http://www.scala-lang.org/node/1305)
* [Learning Scala in Small Bites](http://matt.might.net/articles/learning-scala-in-small-bites/)

본 가이드는 살아있는 문서이므로, 우리들의 최신 "모범사례"를 반영하기 위해 변경될지도 모른다.하지만, 핵심이 되는 아이디어는 아마도 변함이 없을것이다 : 언제나 가독성을 최 우선으로 하라; 汎用的なコードを書き、しかし明瞭さを損なわないこと; シンプルな言語機能を活用せよ。シンプルさは偉大な力をもたらし、また（特に型システムにおける）難解さを回避できる。とりわけ、常にトレードオフを意識しよう。洗練された言語は複雑な実装を要求し、複雑さは複雑さを生む。推論の複雑さ、意味論の複雑さ、機能間相互作用の複雑さ、そして君の協力者への理解の複雑さを。したがって、複雑さは洗練がもたらす税金であり、効用がコストを上回っていることを常に確認すべきだ。
